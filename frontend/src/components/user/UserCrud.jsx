		...
		return this.state.list.map(user => {
			return (
				<tr key={user.id}>
					...
					<td>{user.name}</td>
					<td>{user.email}</td>
					<td><button className="btn btn-waring" onClick={() => this.load(user)}><i className="fa fa-pencil"></i></button><button className="btn btn-danger ml-2" onClick={() => this.remove(user)}><i className="fa fa-trash"></i></button></td>
				</tr>
			)
		})
	}
